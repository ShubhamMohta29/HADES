...
# ── Notes ───────────────────────────────────────────────────────────────────
# All note functions accept an optional user_id. When user_id is provided and
# Supabase is configured they use the cloud DB; otherwise they use notes.txt.
...
def read_notes(category: str = None, user_id: str = None) -> str:
    if _use_db(user_id):
        ...
        try:
            ...
            if not rows:
                return (f"No {category} notes found, Sir." if category else "No notes found, Sir.")
