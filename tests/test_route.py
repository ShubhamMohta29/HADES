"""Unit tests for router.route() — no real voice, API, or file I/O calls.
...
_STUBS = [
    ...
    "webview",
    "vision",
    "services.weather", "services.news", "services.stocks", "services.spotify",
]
for _mod in _STUBS:
...
# Stub voice package so MIC_ERROR sentinel exists
_voice = MagicMock()
_voice.MIC_ERROR = object()
sys.modules["voice"] = _voice
sys.modules["voice.tts"]  = MagicMock()
sys.modules["voice.stt"]  = MagicMock()
sys.modules["voice.wake"] = MagicMock()

# Stub face_auth and brain
sys.modules.setdefault("face_auth", MagicMock())
sys.modules.setdefault("brain", MagicMock())
...
def reset_pending(monkeypatch):
    ...
    import router
    monkeypatch.setattr(router, "_pending_state", {})
...
def test_clear_memory(gui):
    with patch("router.clear_memory", return_value="Memory cleared, Sir.") as m:
...
def test_help_does_not_trigger_on_homework(gui):
    ...
    with patch("router.SCREEN_WORDS", ("help me with my homework",)):
...
def test_weather_with_city(gui):
    with patch("router._weather", return_value="18°C and sunny in London.") as m:
...
def test_weather_default_city(gui):
    with patch("router._weather", return_value="18°C in Toronto.") as m:
...
def test_news_general(gui):
    with patch("router._news", return_value="Top story: ...") as m:
        result = call_route("give me the news", gui)
    m.assert_called_once_with(None)


def test_news_with_topic(gui):
    with patch("router._news", return_value="AI news...") as m:
...
def test_stock_lookup(gui):
    with patch("router._stock", return_value="Tesla: $250.") as m:
        result = call_route("Tesla stock price", gui)
    m.assert_called_once()


def test_crypto_bitcoin(gui):
    with patch("router._crypto", return_value="Bitcoin: $65,000.") as m:
        result = call_route("what is bitcoin at", gui)
    m.assert_called_once_with("bitcoin")


def test_crypto_ethereum(gui):
    with patch("router._crypto", return_value="Ethereum: $3,000.") as m:
...
def test_spotify_play(gui):
    with patch("router._spotify", return_value="Playing Blinding Lights, Sir.") as m:
...
def test_spotify_skip(gui):
    with patch("router._spotify", return_value="Skipping, Sir.") as m:
...
def test_pc_command_handled(gui):
    with patch("router.handle_command", return_value="Volume set to 50%, Sir.") as m:
...
def test_pc_command_returns_none_falls_to_ai(gui):
    with patch("router.handle_command", return_value=None):
        with patch("router.think", return_value="I don't know.") as m:
...
def test_note_triggers_category_question(gui):
    with patch("router.get_existing_categories", return_value=["personal", "work"]):
        with patch("router._suggest_category", return_value="work"):
...
def test_note_pending_confirmed_with_yes(gui):
    ...
    with patch("router.save_note") as mock_save:
        result = call_route("yes", gui)
    mock_save.assert_called_once_with("call dentist", "personal", user_id=None)
...
def test_note_pending_explicit_category(gui):
    ...
    mock_save.assert_called_once_with("finish report", "work", user_id=None)
    assert "work" in result
...
def test_note_pending_new_category(gui):
    ...
    mock_save.assert_called_once_with("buy HDMI cable", "shopping", user_id=None)
...
def test_fallback_to_brain(gui):
    with patch("router.handle_command", return_value=None):
        with patch("router.think", return_value="That's an interesting question.") as m:
